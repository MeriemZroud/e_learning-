...
{
    #[Route('', name: 'app_admin_profile', methods: ['GET', 'POST'])]
    public function index(Request $request, EntityManagerInterface $entityManager): Response|RedirectResponse
    {
        $user = $this->getUser();
        ...
        if ($request->isMethod('POST')) {
            ...
            // Optional fields: empty values keep existing data untouched.
            if ($firstName !== '') {
            ...
            if ($profileImageFile instanceof UploadedFile) {
                ...
                $newFilename = sprintf('admin_%d_%s.%s', $profileUser->getId(), bin2hex(random_bytes(8)), $extension);
